 ...
 */
export async function fetchWithRetries(url, retries, timeoutMs) {
 ...
 */
export async function poolRequests(urls, executingLimit, fetchRetries, fetchTimeoutMs) {
    const results = new Array(urls.length); // Инициализируем массив для результатов
    const executing = new Set(); // Сет для активных промисов
    let currentUrl = undefined;
    for (let i = 0; i < urls.length; i++) {
        currentUrl = urls[i];
        if (currentUrl === undefined)
            continue; // На всякий случай, если в массиве есть пустые элементы
        // Создаем промис для текущего URL
        const p = fetchWithRetries(currentUrl, fetchRetries, fetchTimeoutMs).then((res) => {
        ...
        if (executing.size >= executingLimit)
