 ...
 */
export function isSerializable(data, options = {}) {
    ...
    if (maxDepth &&
        (typeof maxDepth !== "number" ||
            ...
            maxDepth < 0))
        throw new TypeError(`Invalid option: 'maxDepth' must be a positive integer number. Got: ${typeof maxDepth}`);
    function _validate(value, seen = new WeakSet(), currentDepth = 0) {
        ...
        try {
            ...
            if (Array.isArray(value)) {
                for (const item of value) {
                    ...
                    if (!_validate(item, seen, currentDepth + 1))
                        return false;
                }
            }
            else {
                for (const key in value) {
                    if (Object.hasOwn(value, key)) {
                        ...
                        if (!_validate(value[key], seen, currentDepth + 1))
                            return false;
                    }
                }
            }
